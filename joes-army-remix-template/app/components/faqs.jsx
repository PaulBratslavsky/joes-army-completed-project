  ...
  return (
    <div className="w-full md:w-1/2 lg:w-1/3 px-3 mb-20">
      <div className="relative pt-12 px-6 pb-6 border rounded-2xl">
        ...
        <h3 className="mb-6 text-2xl font-semibold font-heading">{question}</h3>
        <p className="text-gray-500 leading-loose">{answer}</p>
      </div>
    </div>
  );
}
...
export default function Faqs({ data }) {
  const { title, text, singleQuestion } = data;
  return (
    <section className="py-20">
      <div className="container px-4 mx-auto">
        <div className="max-w-3xl mx-auto mb-24 text-center">
          <span className="text-xs text-blue-400 font-semibold">{title}</span>
          <h2 className="mt-8 text-4xl font-semibold font-heading">{text}</h2>
        </div>
